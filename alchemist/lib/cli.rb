...
module Alchemist
  class CLI < Thor
    ...
    option :e, type: :string, required: true, desc: 'Comma-separated .db files containing known ground atoms (evidence), including function definitions.'
    option :r, type: :string, required: true, desc: 'The probability estimates are written to this file.'
    option :q, type: :string, desc: 'Query atoms (comma-separated with no space)  ,e.g., cancer,smokes(x),friends(Stan,x). Query atoms are always open world.'
    ...
    def infer
      ...
      chdir_tmp do |tmpdir|
        ...
        fol_mln_paths = sol_mln_paths.each_with_index.map { |path, i|
        ...
        fol_db_paths = sol_db_paths.each_with_index.map { |path, i|
